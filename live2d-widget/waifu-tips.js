...
function initWidget(waifuPath = "/waifu-tips.json", apiPath = "") {
	...
	function registerEventListener() {
		...
		$(document).on("visibilitychange", function() {
			if (!document.hidden) showMessage("哇，你终于回来了，想你了！", 6000, 9);
		});
	}
	...
	function welcomeMessage() {
		...
		if (location.href == SiteIndexUrl) { //如果是主页
			...
			if (location.hostname == referrer.hostname) text = `欢迎阅读来自崔大大的<span style="color:#0099cc;">『${document.title.split(" - ")[0]}』</span>`;
			...
			text = `欢迎阅读来自崔大大的<span style="color:#0099cc;">『${document.title.split(" - ")[0]}』</span>`;
		}
	...
	var userAction = false,
		hitokotoTimer = null,
		messageTimer = null,
		messageArray = ["已经过了这么久了呀，起来活动活动吧", "使用Chrome可以获得最佳浏览体验哦！", "你都不理我，哼！", "拿小拳拳锤你胸口！"];
